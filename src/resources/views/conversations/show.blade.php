                ...
                <li>
                    <a href="{{ route('conversations.show',$c) }}" class="item {{ $active?'active':'' }}">
                        <div class="item-head label">
                            <span class="dot"></span>
                            <span>「{{ $partnerSide->name }}」さんとの取引画面</span>
                        </div>
...
<div class="panel-header">
    <div class="item-head">
        ...
        <div>
            ...
            <div class="muted">相手：{{ $partner->name }}</div>
        </div>
    </div>
...
<div class="chat-body">
    ...
    <div class="bubble-row" style="justify-content:{{ $mine?'flex-end':'flex-start' }};">
        <div class="bubble {{ $mine?'mine':'' }}">
            ...
            @if($m->image_path)
            <div class="imgbox"><img src="{{ asset('storage/'.$m->image_path) }}" alt="image"></div>
            @endif
            @if($mine)
            <div class="bubble__actions">
                <form action="{{ route('messages.destroy',[$conversation,$m]) }}" method="POST" style="display:inline;">
                    @csrf @method('DELETE')
...
<form id="chatForm" class="chat-form" action="{{ route('messages.store',$conversation) }}" method="POST" enctype="multipart/form-data">
    ...
    <div class="row">
        <textarea id="chatBody" name="body" placeholder="本文（必須・400文字まで）">{{ old('body') }}</textarea>
        <input type="file" name="image" accept=".jpeg,.jpg,.png" style="width:220px;">
